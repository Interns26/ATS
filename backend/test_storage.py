...
upload_file(DEFAULT_BUCKET, test_key, b"Hello from MinIO!", content_type="text/plain")
print(f"Uploaded: {test_key}")

# 3. List files to confirm it's there
files = list_files(DEFAULT_BUCKET)
print(f"Files in bucket: {files}")

# 4. Download it back and check the contents match
content = download_file(DEFAULT_BUCKET, test_key)
...
delete_file(DEFAULT_BUCKET, test_key)
