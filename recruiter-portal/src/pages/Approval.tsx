...
import type { Job, JobComment } from "../services/api";
...
export default function Approval() {
  ...
  // Job comment/review state
  const [comment, setComment] = useState("");
  const [reviewerRole, setReviewerRole] = useState("department_head");
  const [reviewAction, setReviewAction] = useState("sent_back");
  ...
  // Load review history whenever a different job is selected
  useEffect(() => {
    if (!selectedId) {
      setComments([]);
      return;
    ...
    const fetchComments = async () => {
      ...
      try {
        const data = await getJobComments(selectedId);
        setComments(data);
      } catch (err) {
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      const newComment = await addJobComment(selectedId, {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        ...
        subtitle={
          activeTab === "pending"
            ? "Select a pending job to review and approve."
            : "Viewing all approved and unapproved job postings."
        }
      >
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-5 text-sm">
              ...
              <div>
                ...
                <StatusBadge
                  isApproved={selectedJob.is_approved}
                />
              </div>
            </div>
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 pt-5">
              ...
              <textarea
                ...
                rows={4}
                className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="bg-slate-700 text-white text-sm font-semibold rounded-lg px-5 py-2.5 hover:bg-slate-800 disabled:opacity-50"
              >
                {commentSubmitting
                  ? "Saving..."
                  : "Submit Review"}
            ...
            {/* Already Approved */}
            {selectedJob.is_approved && (
              <div className="text-xs font-semibold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-800 rounded-lg p-3">
                ✓ This job position is already approved and
                its MinIO storage bucket (
                {selectedJob.minio_bucket}) is active.
              </div>
            )}
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 pt-5">
              ...
              {commentsLoading ? (
                ...
                <div className="border border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-5 text-center text-sm text-slate-400">
                ...
                <div className="space-y-3">
                  {comments.map((item) => (
                    ...
                    >
                      ...
                      <span className="inline-block bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-full px-2.5 py-1 text-xs font-semibold">
                        {item.action}
                      </span>
                    </div>
                  ))}
